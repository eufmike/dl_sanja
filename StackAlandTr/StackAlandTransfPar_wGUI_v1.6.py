...
def worker_load(imgs,i,chunksize,out_q,stfolder):
       """ The worker function, invoked in a process. 'images' is a
           list of images to span the process upon. The results are placed in
           a dictionary that's pushed to a queue.
       """
...
#           print(i*chunksize+imn)
           outdict[i*chunksize+imn] = load_image_from_folder(imgs[imn],stfolder)
       out_q.put(outdict)       
...
#            ccmat.append(tmp[i].get()[1])
    ...
    def load_images_from_folder(self,stfolder,fext):
        ...
        if os.cpu_count()==1:
            nprocs=1
        elif 1<os.cpu_count()<10:
            nprocs=os.cpu_count()-1
        else:
            nprocs=10
        images=self.mp_process_limg(nprocs,fnames,stfolder)                
    ...
    def mp_process_limg(self,nprocs,fnames,stfolder):
        out_q = Queue()
        chunksize = int(math.ceil((len(fnames)-1) / float(nprocs)))
        procs = []
        print("loading images in chunks of size:",chunksize)
        for i in range(nprocs):
            if i == nprocs-1:
                p = Process(
                        target=worker_load,
                        args=(fnames[chunksize * i:len(fnames)],i,chunksize,out_q,stfolder))
                procs.append(p)
                p.start()
                self.loading.progress2['value']+=chunksize
                self.update()
                ...
                p = Process(
                        target=worker_load,
                        args=(fnames[chunksize * i:chunksize * (i + 1)+1],i,chunksize,out_q,stfolder))
                procs.append(p)
                p.start()
                self.loading.progress2['value']+=chunksize
                self.update()
