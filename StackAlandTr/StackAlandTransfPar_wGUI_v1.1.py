...
def alignSIFT(im0,im1,fik,ntree,cr,red,dsc,mtch,imn):
    ...
    im0r,im1r = misc.imresize(im0, 1/red),misc.imresize(im1, 1/red)
#    print(i)
    det=dsc
    matc=mtch
...
class App(tk.Frame):
    def __init__(self,master):
        ...
        self.descr = tk.OptionMenu(self.model_frame, self.descstr, 'ECC', 'sift', 'surf', 'orb', 'akaze','brisk')
...
#            ccmat.append(tmp[i].get()[1])
    ...
    def mp_process(self,nprocs,func,*args):
        images=args[0]
        def worker(imgs,i,chunksize, out_q):
            ...
            for imn in range(len(imgs)):
                ...
                outdict[i*chunksize+imn] = func(images[i*chunksize+imn],images[i*chunksize+imn+1],*args[1:],i*chunksize+imn)
            out_q.put(outdict)
        ...
        for i in range(nprocs):
            if i == nprocs-1:
                p = Process(
                        target=worker,
                        args=(images[chunksize * i:len(images)-1],i,chunksize,
                              out_q))
                procs.append(p)
                p.start()
                ...
                p = Process(
                        target=worker,
                        args=(images[chunksize * i:chunksize * (i + 1)],i,chunksize,
                              out_q))
                procs.append(p)
                p.start()
                self.loading.progress2['value']+=chunksize
