...
def save_tif(folder,subdir,imname,im,suffix,ext):
    misc.imsave(os.path.join(folder,subdir,imname.split('.')[0]+'_'+suffix+ext),im)
    return

def den_Breg(img,wt,itn,epsv,iso,stfolder,sfn,fname,prev,ext):
...
def den_Ch(img,wt,itn,epsv,p3D,stfolder,sfn,fname,prev,ext):
    tmp=restoration.denoise_tv_chambolle(img,wt,n_iter_max=itn,eps=epsv,multichannel=p3D)
    if p3D==True and prev=='N':
        corr=[]
        for i in range(len(tmp)):
            tmp2=misc.toimage(255*tmp[i],cmin=0,cmax=255)
            corr.append(tmp2)
    else:
        corr=misc.toimage(255*tmp,cmin=0,cmax=255)
    if prev=='N':
...
def den_BlF(img,ws,sc,ss,binsv,p3D,stfolder,sfn,fname,prev,ext):
    tmp=restoration.denoise_bilateral(img,win_size=ws,sigma_color=sc,sigma_spatial=ss,bins=binsv,mode='constant',cval=0,multichannel=p3D)
...
def den_nlm(img,ps,pd,hv,p3D,fm,stfolder,sfn,fname,prev,ext):
    tmp=restoration.denoise_nl_means(img,patch_size=ps,patch_distance=pd,h=hv,multichannel=p3D,fast_mode=fm)
    if p3D==True and prev=='N':
        corr=[]
        for i in range(len(tmp)):
            tmp2=misc.toimage(255*tmp[i],cmin=0,cmax=255)
...
class App(tk.Frame):
    ...
    def make_prev_win(self,counter,imgarray,ltext,txttitle,funct,*args):
        ...
        self.prev_c_orig[counter].create_image(0,0,image=self.figp[counter],anchor=tk.NW)
        ...
        self.prev_c_den[self.wcounter].create_image(0,0,image=self.figden[self.wcounter],anchor=tk.NW)
        return
    ...
    def preview_window_TVBr(self,text,entries):
        ...
        self.make_prev_win(self.wcounter,imgarray,texttolabel,text,den_Breg,imgarray,wt,itn,epsv,iso,stfolder,sfn,'','Y',ext)
        return
    ...
    def preview_window_TVCh(self,text,entries):
        ...
        self.make_prev_win(self.wcounter,imgarray,texttolabel,text,den_Ch,imgarray,wt,itn,epsv,p3D,stfolder,sfn,'','Y',ext)
        return
    ...
    def preview_window_BlF(self,text,entries):
        ...
        self.make_prev_win(self.wcounter,imgarray,texttolabel,text,den_BlF,imgarray,ws,sc,ss,bins,p3D,stfolder,sfn,'','Y',ext)
        return
    ...
    def preview_window_NLM(self,text,entries):
        ...
        self.make_prev_win(self.wcounter,imgarray,texttolabel,text,den_nlm,imgarray,ps,pd,h,p3D,fm,stfolder,sfn,'','Y',ext)
        return
...
#        global wt,itn,eps,iso
        ...
        if p3D==True:
            ...
            den_Ch(stack,wt,itn,eps,p3D,stfolder,sfn,fnames,'N',ext)
        end = time.time()
        secs = end-start
        print("Filternig took", secs)
        print('successfully filtered all the images')
...
#        global wt,itn,eps,iso
        ...
        if p3D==True:
            if not sfn in os.listdir(stfolder):
                os.makedirs(os.path.join(stfolder,sfn))
                print('directory created')
            den_BlF(stack,ws,sc,ss,bins,p3D,stfolder,sfn,fnames,'N',ext)
        end = time.time()
        secs = end-start
        print("Filternig took", secs)
        print('successfully filtered all the images')
...
#        global wt,itn,eps,iso
        ...
        if p3D==True:
            if not sfn in os.listdir(stfolder):
                os.makedirs(os.path.join(stfolder,sfn))
                print('directory created')
            den_nlm(stack,ps,pd,h,p3D,fm,stfolder,sfn,fnames,'N',ext)
        end = time.time()
        secs = end-start
        print("Filternig took", secs)
        print('successfully filtered all the images')
    ...
    def TVBden_multi_func(self,imstack,wt,itn,eps,iso,nCORES,stfolder,sfn,fnames,ext):
        ...
        for i in range(len(imstack)):
            results.append(pool.apply_async(den_Breg,(imstack[i],wt,itn,eps,iso,stfolder,sfn,fnames[i],'N',ext,)))
            results[0].get()
            self.loading.progress2['value']+=1
            self.update()
        pool.close()
    ...
    def TVChden_multi_func(self,imstack,wt,itn,eps,p3D,nCORES,stfolder,sfn,fnames,ext):
        ...
        results=[]
        for i in range(len(imstack)):
            results.append(pool.apply_async(den_Ch,(imstack[i],wt,itn,eps,p3D,stfolder,sfn,fnames[i],'N',ext,)))
            self.loading.progress2['value']+=1
            self.update()
        pool.close()
        pool.join()
    ...
    def BlFden_multi_func(self,imstack,ws,sc,ss,bins,p3D,nCORES,stfolder,sfn,fnames,ext):
        ...
        results=[]
        for i in range(len(imstack)):
            results.append(pool.apply_async(den_BlF,(imstack[i],ws,sc,ss,bins,p3D,stfolder,sfn,fnames[i],'N',ext,)))
            self.loading.progress2['value']+=1
            self.update()
        pool.close()
        pool.join()
    ...
    def NLMden_multi_func(self,imstack,ps,pd,h,p3D,fm,nCORES,stfolder,sfn,fnames,ext):
        ...
        results=[]
        for i in range(len(imstack)):
            results.append(pool.apply_async(den_nlm,(imstack[i],ps,pd,h,p3D,fm,stfolder,sfn,fnames[i],'N',ext,)))
            self.loading.progress2['value']+=1
            self.update()
        pool.close()
        pool.join()
