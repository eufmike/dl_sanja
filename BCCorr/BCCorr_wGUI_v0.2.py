...
def imgcorrection(imtbc,imref,redf,gaussks,fname,fext,stfolder,sfn,nCORES,rev):
    imtbcr,imrefr=denandred(imtbc,redf,gaussks),denandred(imref,redf,gaussks)
#        im1r,im0r=denandred(im1,redf,25),denandred(im0,redf,25)
    vref=im2vec(imrefr)
    vtbc=im2vec(imtbcr)
    imtbcave,imtbcst,=np.average(vtbc),np.std(vtbc)
    imrefave,imrefst,=np.average(vref),np.std(vref)
    a=imrefst/imtbcst
    b=imrefave-a*imtbcave
    cimg=a*imtbc+b
    corrimg=misc.toimage(cimg,cmin=0,cmax=255)
    save_tif(stfolder,sfn,fname,corrimg,fext)
    return a,b,cimg

#corr=stack[9]*np.std(stack[10])/np.std(stack[9])-np.average(stack[9])*np.std(stack[10])/np.std(stack[9])+np.average(stack[10])
...
#        self.cld[self.wcounter]=clahefunct2(imgarray,ts,cl,nb,stfolder,sfn,ext,'','Y')
    ...
    def corrimgint(self,imstack,fnames,ind,redf,gaussks,nCORES,fext,stfolder,sfn):
        ...
        for i in range(ind,len(imstack)-1):
            results=[]
            results=imgcorrection(imstack[i+1],cprev,redf,gaussks,fnames[i+1],fext,stfolder,sfn,nCORES,rev)
            self.loading.progress2['value']+=1
            self.update()
            amat.append(results[0])
            bmat.append(results[1])
            cprev=np.copy(results[2])
        results=imgcorrection(imstack[ind],imstack[ind],redf,gaussks,fnames[ind],fext,stfolder,sfn,nCORES,rev)
        amat.insert(0,results[0])
        bmat.insert(0,results[1])
        print(fnames[ind],ind,results[0],results[1])
        rev=1
        for i in range(ind,0,-1):
            results=[]
            results=imgcorrection(imstack[i-1],cnext,redf,gaussks,fnames[i-1],fext,stfolder,sfn,nCORES,rev)
            self.loading.progress2['value']+=1
            self.update()
            amat.insert(0,results[0])
            bmat.insert(0,results[1])
            cnext=np.copy(results[2])
        amat,bmat=np.array(amat),np.array(bmat)
        return amat,bmat
    ...
    def CorrInt(self,entries):
        ...
        ststd=[]
        for i in stack:
            stave.append(np.average(i))
            ststd.append(np.std(i))
        print('Average of each image of the stack')
        plt.scatter(linspace(0,len(stave),len(stave)),stave)
        plt.show()
        print('StDev of each image of the stack')
        plt.scatter(linspace(0,len(stave),len(stave)),ststd)
        plt.show()
        end = time.time()
        secs = end-start
        ...
        self.update()        
        am,bm=self.corrimgint(stack,fnames,rimind,red,gks,nC,ext,stfolder,sfn)
        end = time.time()
        secs = end-start
