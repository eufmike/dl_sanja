...
def fetch_data_1dir(path,fext):
    training_data_files_list = list()
    for f in sorted(os.listdir(path)): 
        if fext in f:
            training_data_files_list.append(os.path.join(path,f))
    return training_data_files_list

def fetch_data_subdirs(path,fext):
    training_data_files_list = list()
    tmp=[]
    for subfolder in [name for name in sorted(os.listdir(path)) if os.path.isdir(os.path.join(path, name))]:
        for f in sorted(os.listdir(os.path.join(path,subfolder))):
            if fext in f:                
                tmp.append(os.path.join(path,subfolder,f))
        training_data_files_list.append(tmp)
        tmp=[]
    return training_data_files_list

def fetch_augdata_subdirs(path,fext):
    training_data_files_list = list()
    tmp=[]
    for subfolder in [name for name in sorted(os.listdir(path)) if os.path.isdir(os.path.join(path, name))]:
        for subsubfolder in [name for name in sorted(os.listdir(os.path.join(path,subfolder))) if os.path.isdir(os.path.join(path, subfolder, name))]:
            for f in sorted(os.listdir(os.path.join(path,subfolder,subsubfolder))):
                if fext in f:                
                    tmp.append(os.path.join(path,subfolder,subsubfolder,f))
            training_data_files_list.append(tmp)
            tmp=[]
    return training_data_files_list
...
#    print('image shape:',image_shape)
    ...
    if normalize:
        normalize_data_storage(storage)
    hdf5_file.close()
    return filters

def write_data_to_file(file_list, out_file, imtype, truth_dtype=np.uint8, is3D=False, subject_ids=None,
                       normalize=False, crop=False):
    """
    Takes in a set of training images and writes those images to an hdf5 file.
    :param file_list: List containing the data files. 
    ...
    if is3D==True:
        ...
        for file in file_list[0]:
            vol.append(read_image(file))
        image_shape = np.array(vol).shape
    else:
...
#        os.remove(out_file)
        raise e
    write_image_data_to_file(file_list, storage, image_shape, n_channels,is3D=is3D)
    if subject_ids:
        hdf5_file.create_array(hdf5_file.root, 'subject_ids', obj=subject_ids)
    if normalize:
...
def write_image_data_to_file(in_files, data_storage, image_shape, n_channels, truth_dtype=np.uint8, crop=False, is3D=False):
    if is3D:
        images = read3D_image_files(in_files)
        subject_data = [volume for volume in images]
...
def read3D_image_files(image_subf_list):
    image_list=list()
    for subf in image_subf_list:
        image_list.append(read_image_files(subf))
    return image_list
...
def distort_image(d,t, flip_axis=None, scale_factor=None):
    ...
    else: trd,trt = flip_image(d,t,np.random.randint(0,3,1))
    return trd,trt
...
def print_hdf5_item_structure(filename):
