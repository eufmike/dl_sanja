...
from data import load_train_data, load_test_data, crop_no_black, preprocess,preprocess3D, getSegmentationArr, fetch_data_1dir, write_data_to_file, write_3Ddata_to_file, open_hdf5_file, read_image, extract_3D_patches, sweep_3D_patches,transform_3Dpatch, fetch_data_subdirs, fetch_augdata_subdirs
import Nmodels
import Nmodels3D
...
config['tr_p']='train'
config['test_p']='test'
config['3Ddatafolder']='3D'
config['selfold']='Augmented'
config['3Daugm']='3DAugmented'
...
aug3Dfol=config['3Daugm']
datasubf3D=config['3Ddatafolder']
...
#        create_train_data(data_path,os.path.join(image_p,tmpf),os.path.join(label_p,tmpf),fext)
        imhdf5=open_hdf5_file(config['image_hdf5_path'])
        tmptr=np.squeeze(imhdf5.root.data,axis=(-1))
        imhdf5.close()
        labhdf5=open_hdf5_file(config['label_hdf5_path'])
        tmplab=np.squeeze(labhdf5.root.truth,axis=(-1))
        labhdf5.close()
        print(imgshape,cr)
...
#            imgsopen=open_hdf5_file(outtest)
#            imgs_train = np.squeeze(imgsopen.root.test,axis=(-1))
#            print('hdf test data shape:',imgs_train.shape)
#            imgsopen.close()
...
#        imgsopen.close()
    ...
    def TrainModel(self,TMentries):
        ...
        imgs_mask_train = np.squeeze(labopen.root.truth,axis=(-1))
        labopen.close()
...
#            imgs_mask_train2 = np.copy(imgs_mask_train)
    ...
    #    model = Model4(imgs_train[0].shape)
        print(os.path.join(data_path,model_p))
        if os.path.exists(os.path.join(data_path,model_p)) and len([x for x in os.listdir(os.path.join(data_path,model_p)) if ('.hdf5') in x])>0:
            ...
            print('using weights from:',latest)
    ...
    #    model_checkpoint = ModelCheckpoint('weights.h5', monitor='val_loss', save_best_only=True) 
        ...
        plot_model(model, to_file=os.path.join(data_path,model_p,'model_plot.png'), show_shapes=True, show_layer_names=True)
        print('-'*30)
        print('Fitting model...')
        print('-'*30)
    ...
    def TrainModel3D(self,TMentries):
        ...
        print('-'*30)
        imgsopen=open_hdf5_file(config['image_hdf5_path'])
        imgs_train_tot = np.squeeze(imgsopen.root.data,axis=(-1))
        print('Train dataset shape:',imgs_train_tot.shape)
        imgsopen.close()
        labopen=open_hdf5_file(config['label_hdf5_path'])
        imgs_mask_train_tot = np.squeeze(labopen.root.truth,axis=(-1))
        labopen.close()
        imgs_train=[]
        imgs_mask_train=[]
        for vol in imgs_train_tot:
            imgs_train.append(vol)
        for vol in imgs_mask_train_tot:
            imgs_mask_train.append(vol)
        imgs_train=np.array(imgs_train)
        imgs_mask_train=np.array(imgs_mask_train)
#        print('new dataset size:',imgs_mask_train.shape)
...
#            imgs_mask_train2 = np.copy(imgs_mask_train)
    ...
    def Predict(self,TMentries):
        ...
        imgs_test_tot = np.squeeze(test_open.root.test,axis=(-1))
        test_open.close()
...
#                    imagep = (image * 255.).astype(np.uint8)
    ...
    def Predict3D(self,TMentries):
        ...
        print('Shape of the stack for predictions:',imgs_test_tot.shape)
        print('-'*30)
        print('Loading saved weights...')
        print('-'*30)
        print(max(glob.glob(os.path.join(data_path,model_p,'*.hdf5')),key=os.path.getctime))
        ...
        if not GPU:
            ...
            if nLab>1:
                ...
                vol_mask = np.squeeze((imgs_mask_test * 255.).astype(np.uint8),axis=(-1))
        else:
            p_size=128
            if Overlap:
                if imgs_test_tot.shape[0]<=p_size:
                    imgs_test_totshft=np.roll(np.roll(imgs_test_tot,p_size//2,axis=2),p_size//2,axis=1)
                else:
                ...
                if nLab>=1:
                    volmask1=volmask1.reshape(img_depth, img_rows, img_cols)
                    volmask2=volmask2.reshape(img_depth, img_rows, img_cols)
                if imgs_test_tot.shape[0]<=p_size:
                    volmask2shft=np.roll(np.roll(volmask2,-p_size//2,axis=2),-p_size//2,axis=1)
                else:
                    volmask2shft=np.roll(np.roll(np.roll(volmask2,-p_size//2,axis=2),-p_size//2,axis=1),-p_size//2,axis=0)
                if nLab==1:
                    vol_mask=(volmask1+volmask2shft)/2
                    vol_mask[vol_mask>0.35]=255
                if nLab>1:
                    tmp=[]
                    for i in range(1,nLab):
                        tmp1=np.ma.masked_not_equal(volmask1,i).filled(0)/i
                        tmp2=np.ma.masked_not_equal(volmask2shft,i).filled(0)/i
                        tmp3=((tmp1+tmp2)/2)
                        tmp3[tmp3>0.4]=i
                        tmp.append(tmp3)
                    tmp=np.array(tmp)
                    vol_mask=np.sum(tmp,axis=0)

    #            vol_mask=volmask2shft
            else:
                vol_mask=self.pred_vol(imgs_test_tot,nLab,mName,p_size,latest)
                if nLab==1:
                    vol_mask[vol_mask>0.35]=255
